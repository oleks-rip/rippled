...
NotTEC
DepositPreauth::preauthPreflightCheck(
    ...
    beast::Journal const j)
{
    if (ctx.tx.isFieldPresent(sfCredentialIDs))
    {
        ...
        STVector256 const& arr(ctx.tx.getFieldV256(sfCredentialIDs));
        if (arr.empty() || (arr.size() > credentialsArrayMaxSize))
        {
            JLOG(j.trace())
                << "Malformed transaction: Credentials array is empty.";
            return temMALFORMED;
        }
    }
    return tesSUCCESS;
    ...
    beast::Journal const j)
{
    ...
    bool const ruleAuth = view.rules().enabled(featureDepositAuth);
    bool const rulePreAuth = view.rules().enabled(featureDepositPreauth);
    bool const reqAuth =
        sleDst && (sleDst->getFlags() & lsfDepositAuth) && ruleAuth;
    bool const credentialsPresent = tx.isFieldPresent(sfCredentialIDs);

    if (reqAuth && !rulePreAuth)
    {
        JLOG(j.trace()) << "Preauth rule is disabled.";
        return tecNO_PERMISSION;
    }

    if (credentialsPresent && (!reqAuth || (src == dst)))
    {
        JLOG(j.trace()) << "Destination doesn't not require authorization.";
        return tecNO_PERMISSION;
    }
    ...
    {
        auto const err = preauthPreclaimCredentialsCheck(
            view, src, dst, tx.getFieldV256(sfCredentialIDs), j);
        if (!isTesSuccess(err))
            return err;
    }
    else if (!view.exists(keylet::depositPreauth(dst, src)))
    {
...
// Not used in Payment tx
TER
DepositPreauth::preauthApplyCheck(
    ApplyView& view,
    STTx const& tx,
    AccountID const& src,
    AccountID const& dst,
    ...
    beast::Journal const j)
{
    ...
    // authorization has 2 ways to get an XRP Payment in:
    //  1. If Account == Destination, or
    //  2. If Account is deposit preauthorized by destination. This already
    //  checked in preauthPreclaimCheck
    ...
    {
        if (credentialIDsRemoveExpired(view, tx, j))
            return tecEXPIRED;
    }

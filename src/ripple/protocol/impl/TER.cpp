...
#define MAKE_ERROR(code, desc) { code, { #code, desc } }
            ...
            std::pair<char const* const, char const* const>> const results
    {
        ...
        MAKE_ERROR(tecXCHAIN_CLAIM_NO_QUORUM,        "xchain claim no quorum."),
        ...
        MAKE_ERROR(temEQUAL_DOOR_ACCOUNTS,       "Malformed: Bridge must have unique door accounts."),
        MAKE_ERROR(temBAD_XCHAIN_PROOF,          "Malformed: Bad cross-chain claim proof."),
        MAKE_ERROR(temBRIDGE_BAD_ISSUES,      "Malformed: Bad bridge issues."),
        MAKE_ERROR(tem_BRIDGE_NONDOOR_OWNER,   "Malformed: Bridge owner must be one of the door accounts."),

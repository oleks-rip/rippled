...
struct XChainBatch_test : public beast::unit_test::suite,
                          public jtx::XChainBridgeObjects
{
    void
    testCreateAccountIssue()
    {
        using namespace jtx;
    ...
    void
    testAttestationBatch()
    {
        using namespace jtx;
        testcase("Add Attestation Batch");
        XRPAmount res0 = reserve(0);
        XRPAmount tx_fee = txFee();
        STAmount tx_fee_2 = multiply(tx_fee, STAmount(2), xrpIssue());
        ...
        for (auto withClaim : {false})
        {
            ...
            BEAST_EXPECT(transfer.payees_received(multiply(
                split_reward_everyone,
                STAmount(2),
                split_reward_everyone.issue())));
    ...
    testXChainClaim()
    {
        ...
        // This also verifies that if a batch of attestations
        // brings the signatures over quorum (say the quorum is 4 and there are
        // 5 attestations) then the reward should be split among the five
        // accounts.
        // -----------------------------------------------------------------
        for (auto withClaim : {false, true})
        {
...
BEAST_DEFINE_TESTSUITE(XChainBatch, app, ripple);
